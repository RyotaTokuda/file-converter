...
import { formatBytes } from "./imageConverter";
...
export interface VideoConvertOptions {
  ...
  videoBitrate: string; // e.g., "1500k" — empty means use CRF
  maxWidth: string;     // e.g., "1280" — empty means no limit
  fps: string;          // for GIF only, e.g., "15"
}
...
export function preloadFFmpeg(): void {
  getFFmpeg().catch(() => {/* silently ignore preload failure */});
}
...
export async function convertVideo(
  ...
  try {
    ...
    // readFile returns Uint8Array | string; copy to plain ArrayBuffer to avoid SharedArrayBuffer issues
